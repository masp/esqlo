...
// A fork with modifications of https://github.com/hoisie/mustache/blob/6375acf62c69d9d3ad20fd0599d82ca94ea12284/mustache.go
// - All functionality related to partials and functions has been removed.
// - The elements in the template are visible and part of the API.
...
Outer:
	for _, ctx := range contextChain { //i := len(contextChain) - 1; i >= 0; i-- {
		...
		for v.IsValid() {
			...
			case reflect.Slice:
				var idx int
				if name[0] == '[' && name[len(name)-1] == ']' { // e.g. [10]
					v, err := strconv.Atoi(name[1 : len(name)-1])
					if err != nil { // not an index lookup, ignore
						continue Outer
					}
					idx = v
					if idx < 0 || idx >= av.Len() {
					...
					return av.Index(idx)
				} else { // not an explicit index lookup, we assume we should look up in index 0
					if v.Len() == 0 {
						continue Outer
					}
					v := av.Index(0)
					return lookup([]interface{}{v}, name) // retry lookup with the first element of the slice for this name instead
				}
			default:
				continue Outer
			}
...
// normalizeNames converts "foo[0].bar[1]" into "foo.[0].bar.[1]" and "foo.bar" into "foo.bar".
func normalizeNames(s string) (r string) {
	for j := 0; j < len(s); j++ {
		if s[j] == '[' {
			r += "."
		}
...
func renderElement(element interface{}, contextChain []interface{}, buf io.Writer) {
	...
	case *textElement:
		buf.Write(elem.text)
	case *varElement:
		name := normalizeNames(elem.name)
		defer func() {
			if r := recover(); r != nil {
				fmt.Printf("Panic while looking up %q: %s\n", elem.name, r)
			}
		}()
		val := lookup(contextChain, name)
